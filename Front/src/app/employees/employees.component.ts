...
console.log(this.userForm)
    try {
      const response = await this.http.post('http://localhost:5000/api/employee', this.userForm.value).toPromise();
  ...
  this.congeForm.patchValue({
    ...
    this.imageSrc = `http://localhost:5000/img/${employee.image}`;
  }
  ...
  async onUpdate() {
    ...
    try {
      const response = await this.http.put(`http://localhost:5000/api/employee/${this.selectedEmployee.matricule}`, this.updateForm.value).toPromise();
  ...
  async removeEmployee() {
    try {
      await this.http.get(`http://localhost:5000/api/employee/toggleEmployeeStatus/${this.selectedEmployee.matricule}`).toPromise();
  ...
  onSubmit1(): void {
    ...
    this.http.post<any>('http://localhost:5000/api/authorization', this.authForm.value)
      .subscribe(
        (response) => {
  ...
  onSubmit2(): void {
    ...
    // Envoyer la requête HTTP pour ajouter l'autorisation
    this.http.post<any>('http://localhost:5000/api/conge', this.congeForm.value)
      .subscribe(
        (response) => {
          console.log(response);
...
async exportToExcel(): Promise<void> {
  ...
  for (const employee of this.employees) {
    ...
    try {
       const response2 = await this.http.get<any>('http://localhost:5000/api/employeePoints/getEmployeePointsDetails/' + this.removeLeadingZeros(employee.matricule)).toPromise();
       const response3 = await this.http.get<any>('http://localhost:5000/api/authorization/' + employee.matricule).toPromise();
